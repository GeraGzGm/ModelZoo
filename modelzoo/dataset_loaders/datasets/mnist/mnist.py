...
class MNIST(BaseDataset):
    LABELS = Labels
    NUM_WORKERS = 4
    def __init__(self, train_batch_size: int, inference_batch_size: int, root: str = "./_datasets/"):
        self.root = root
    ...
    def get_datasets(self, train_transforms: list[dict[str, str]], test_transforms: list[dict[str, str]], split_ratio: list[float]) -> tuple[DataLoader, Optional[DataLoader], DataLoader]:
        ...
        dataset = AlbumentationsWrapper( datasets.MNIST(self.root, train = True, download = True), transform = train_transforms )
        train_dataset, val_dataset = self._split_train(dataset, split_ratio)

        test_dataset = AlbumentationsWrapper( datasets.MNIST(self.root, train = False, download = True), transform = test_transforms )
